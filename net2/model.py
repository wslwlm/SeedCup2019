...
class My_MSE_loss_1(nn.Module):
    ...
    def forward(self, x, y):
        return torch.mean(torch.where(x > y, torch.pow((x - y), 2) * 9, 
                torch.pow((x - y), 2)))
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        #output_encoder_payed_hour = self.encoder_payed_hour(x[:,15].long())

        concat_encoder_output = torch.cat((
        output_encoder_biz_type,
        output_encoder_cate1_id, output_encoder_cate2_id,
        output_encoder_cate3_id, output_encoder_seller_uid,
        output_encoder_company_name,
        output_encoder_rvcr_prov_name,
