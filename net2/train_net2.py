...
from model import Network,My_MSE_loss,My_MSE_loss_1
...
criterion_2 = My_MSE_loss_1()
criterion_3 = My_MSE_loss()
...
def train(epoch):
    ...
    #optimizer = optim.SGD(net.parameters(), lr=opt.get_lr(epoch), momentum=0.9)
    for batch_idx, (inputs, targets_sign_day, targets_sign_hour, targets_ship_day, targets_ship_hour, targets_got_day, targets_got_hour, targets_dlved_day, targets_dlved_hour, targets_ship_pay_day, targets_pre_day) in enumerate(tqdm(trainloader)):
        if opt.USE_CUDA:
            inputs = inputs.cuda()
            ...
            targets_ship_pay_day = targets_ship_pay_day.cuda()
            targets_pre_day = targets_pre_day.cuda()
        ...
        targets_ship_pay_day = torch.autograd.Variable(targets_ship_pay_day.float())
        #targets_ship_pay_day = torch.autograd.Variable(targets_ship_pay_day.long())
        targets_pre_day = torch.autograd.Variable(targets_pre_day.float())
        ...
        # 对预售时间和签收时间的时间差进行预测
        loss_1_1 = criterion_2(output_FC_1_1, targets_pre_day)
        #loss_2_1 = criterion_4(output_FC_2_1, targets_ship_pay_day)
        ...
        #loss_2_2 = criterion_4(output_FC_2_2, targets_ship_pay_day)
        #loss_3_2 = criterion_1(output_FC_3_2, targets_got_hour)
        #loss_4_2 = criterion_1(output_FC_4_2, targets_dlved_hour)

        loss_day  = loss_1_1
        loss_hour = loss_1_2
        #loss_ship_pay = loss_2_1

        loss = (loss_day + loss_hour) 
        #loss = loss_1_1 + loss_1_2 + loss_2_1 + loss_2_2 + loss_3_1 + loss_3_2 + loss_4_1 + loss_4_2
        loss.backward()
        ...
        if batch_idx == 1:
            ...
            #print("==> epoch {}: loss_day is {}, loss_hour is {}, loss_ship_pay: {}".format(epoch, loss_day, loss_hour, loss_ship_pay))
...
def val(epoch):
    ...
    pred_signed_time = []
    real_signed_time = []
    for batch_idx, (inputs, shipped_time, dlved_time, signed_time, preselling_shipped_time) in enumerate(tqdm(valloader)):
        if opt.USE_CUDA:
            inputs = inputs.cuda()
        ...
        for i in range(len(inputs)):
            ...
            temp_payed_time = preselling_shipped_time[i]
            #temp_payed_time = shipped_time[i]
    ...
    # save model
    if rankScore_result < highest_metrics and onTimePercent_result > 0.982:
        print("==> saving model")
