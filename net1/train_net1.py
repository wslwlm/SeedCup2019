...
def train(epoch):
    ...
    for batch_idx, (inputs, targets_sign_day, targets_sign_hour, targets_ship_day, targets_ship_hour, targets_got_day, targets_got_hour, targets_dlved_day, targets_dlved_hour, targets_ship_pay_day) in enumerate(tqdm(trainloader)):
        if opt.USE_CUDA:
            inputs = inputs.cuda()
...
def val(epoch):
    ...
    pred_signed_time = []
    real_signed_time = []
    for batch_idx, (inputs, payed_time, shipped_time, dlved_time, signed_time) in enumerate(tqdm(valloader)):
        if opt.USE_CUDA:
            inputs = inputs.cuda()
